import { supabaseAdmin, supabase, type Database } from './supabase'
...
type DocumentStats = Tables<'document_stats'>
...
export class SupabaseDocumentService {
  ...
  async createDocumentWithFile(
    ...
    try {
      ...
      // 3. Crear documento en Supabase con URL del archivo
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          file_url: fileUrl,
          file_name: fileName,
          file_size: file instanceof File ? file.size : fileBuffer.byteLength,
          created_by: userId
        })
        .select()
        .single()
  ...
  // Crear documento (solo administradores)
  async createDocument(documentData: Omit<DocumentInsert, 'id' | 'created_at' | 'updated_at'>, userId: string) {
    const { data, error } = await supabaseAdmin
      .from('documents')
      .insert({
        ...documentData,
  ...
  async incrementViews(documentId: string, userIp?: string, userAgent?: string) {
    ...
    if (userIp || userAgent) {
      await this.logActivity('document_viewed', documentId, null, {
        user_ip: userIp,
  ...
  async incrementDownloads(documentId: string, userIp?: string, userAgent?: string) {
    ...
    // Log adicional con información del usuario
    if (userIp || userAgent) {
      await this.logActivity('document_downloaded', documentId, null, {
        user_ip: userIp,
        user_agent: userAgent
      })
    }
  ...
  async getGeneralStats() {
    const [documentsCount, viewsSum, downloadsSum] = await Promise.all([
      supabase.from('documents').select('id', { count: 'exact', head: true }),
      supabase.from('document_stats').select('views').then(({ data }) => 
        data?.reduce((sum, stat) => sum + stat.views, 0) || 0
      ),
      supabase.from('document_stats').select('downloads').then(({ data }) => 
        data?.reduce((sum, stat) => sum + stat.downloads, 0) || 0
      )
    ])

    const { count: totalDocuments } = documentsCount
